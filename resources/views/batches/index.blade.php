...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex gap-3">
            <a href="{{ route('batches.create') }}" class="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 text-sm font-semibold">
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        <form action="{{ route('batches.filter') }}" method="POST" id="filter-form" class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            @csrf
            <div>
                <label for="search" class="block text-sm font-medium text-gray-700 mb-2">اسم الطرد</label>
                <input type="text" name="search" id="search" value="{{ $filters['search'] ?? '' }}" placeholder="ابحث بالاسم أو النوع..."
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="draft" {{ ($filters['status'] ?? '') === 'draft' ? 'selected' : '' }}>جديد</option>
                    <option value="active" {{ ($filters['status'] ?? '') === 'active' ? 'selected' : '' }}>معتمد</option>
                </select>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($batches as $batch)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center">{{ $loop->iteration + ($batches->currentPage() - 1) * $batches->perPage() }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center">{{ $batch->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $batch->type_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $batch->quantity ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $batch->batch_date ? $batch->batch_date->format('Y-m-d') : '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $batch->beneficiaries_count }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-emerald-600">{{ $batch->received_count ?? 0 }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-600">{{ ($batch->beneficiaries_count ?? 0) - ($batch->received_count ?? 0) }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex items-center justify-center gap-3">
                                    @if($batch->status === 'active')
                                        <a href="{{ route('batches.distribution', $batch) }}" 
                                    ...
                                    @else
                                        <a href="{{ route('batches.manage', $batch) }}" 
                                           class="px-3 py-1.5 rounded-lg hover:bg-emerald-700 transition-all text-xs font-semibold whitespace-nowrap"
                                           style="background-color: #16a34a; color: white; text-decoration: none; display: inline-block;"
                                    ...
                                    @endif
                                    <form action="{{ route('batches.destroy', $batch) }}" method="POST" class="inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
        ...
        @if($batches->hasPages())
            <div class="px-6 py-4 border-t border-gray-200">
                <div class="text-sm text-gray-600 text-center mb-2">
                    عرض <strong>{{ $batches->firstItem() ?? 0 }}</strong> إلى <strong>{{ $batches->lastItem() ?? 0 }}</strong> من <strong>{{ $batches->total() }}</strong>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('clear-filters-btn')?.addEventListener('click', function() {
        ...
        form.method = 'POST';
        form.action = '{{ route('batches.filter.reset') }}';
...
function exportExcel() {
    window.location.href = '{{ route('batches.export-excel') }}';
}
</script>
@endpush
@endsection
