<?php $__env->startSection('title', 'المستفيدين'); ?>
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div class="flex items-center gap-3">
            <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'beneficiaries.create')): ?>
            ...
            <a href="<?php echo e(route('beneficiaries.create')); ?>" class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl shadow-lg hover:shadow-xl transition-all font-semibold">
                إضافة مستفيد جديد
            </a>
            <?php endif; ?>
        </div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <div class="flex justify-between items-center">
            <div>
                <p class="text-blue-800 font-semibold">تم اختيار طرد: <?php echo e($batch->name); ?></p>
                <p class="text-blue-600 text-sm mt-1">
                    العدد الحالي في الطرد: <?php echo e($batch->beneficiaries->count()); ?> / <?php echo e($batch->quantity ?? 'غير محدد'); ?>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-4 pt-6">
        <form method="POST" action="<?php echo e(route('beneficiaries.filter')); ?>" id="filter-form" class="space-y-3">
            <?php echo csrf_field(); ?>
            <?php if($batch): ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                    ...
                    <select name="beneficiary_id" id="beneficiary-select" class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        ?>
                        <?php $__currentLoopData = $allBeneficiaries ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $beneficiary): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($beneficiary->id); ?>" <?php echo e(($selectedBeneficiaryId && $beneficiary->id == $selectedBeneficiaryId) ? 'selected' : ''); ?>>
                                <?php echo e($beneficiary->name); ?> 
                                <?php if($beneficiary->national_id): ?> - <?php echo e($beneficiary->national_id); ?> <?php endif; ?>
                                <?php if($beneficiary->phone): ?> - <?php echo e($beneficiary->phone); ?> <?php endif; ?>
                            </option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php if($selectedBeneficiaryId): ?>
                            <?php
                                $selectedBeneficiary = \App\Models\Beneficiary::find($selectedBeneficiaryId);
                            ?>
                            <?php if($selectedBeneficiary && !($allBeneficiaries ?? collect())->contains('id', $selectedBeneficiaryId)): ?>
                                <option value="<?php echo e($selectedBeneficiary->id); ?>" selected>
                                    <?php echo e($selectedBeneficiary->name); ?> 
                                    <?php if($selectedBeneficiary->national_id): ?> - <?php echo e($selectedBeneficiary->national_id); ?> <?php endif; ?>
                                    <?php if($selectedBeneficiary->phone): ?> - <?php echo e($selectedBeneficiary->phone); ?> <?php endif; ?>
                                </option>
                            <?php endif; ?>
                        <?php endif; ?>
                    </select>
                ...
                <div>
                    ...
                    <select name="residence_status" class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="">الكل</option>
                        <option value="resident" <?php echo e(($filters['residence_status'] ?? '') === 'resident' ? 'selected' : ''); ?>>مقيم</option>
                        <option value="displaced" <?php echo e(($filters['residence_status'] ?? '') === 'displaced' ? 'selected' : ''); ?>>نازح</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="relationship" class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="">الكل</option>
                        <option value="زوج/ة" <?php echo e(($filters['relationship'] ?? '') === 'زوج/ة' ? 'selected' : ''); ?>>زوج/ة</option>
                        <option value="أرمل/ة" <?php echo e(($filters['relationship'] ?? '') === 'أرمل/ة' ? 'selected' : ''); ?>>أرمل/ة</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="">الكل</option>
                        <option value="active" <?php echo e(($filters['status'] ?? '') === 'active' ? 'selected' : ''); ?>>موجود</option>
                        <option value="inactive" <?php echo e(($filters['status'] ?? '') === 'inactive' ? 'selected' : ''); ?>>غير موجود</option>
                    </select>
                </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="has_pregnant" value="1" <?php echo e(($filters['has_pregnant'] ?? false) ? 'checked' : ''); ?>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="has_nursing" value="1" <?php echo e(($filters['has_nursing'] ?? false) ? 'checked' : ''); ?>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="has_children" value="1" id="has_children_checkbox" <?php echo e(($filters['has_children'] ?? false) ? 'checked' : ''); ?>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="filter_members" value="1" id="filter_members_checkbox" <?php echo e(($filters['filter_members'] ?? false) ? 'checked' : ''); ?>
                ...
                <div id="age-fields" class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3" style="display: <?php echo e(($filters['has_children'] ?? false) ? 'grid' : 'none'); ?>;">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">الحد الأدنى للعمر</label>
                        <input type="number" name="age_min" value="<?php echo e($filters['age_min'] ?? ''); ?>" min="0" max="120"
                    ...
                    <div>
                        ...
                        <input type="number" name="age_max" value="<?php echo e($filters['age_max'] ?? ''); ?>" min="0" max="120"
                            class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>

                <!-- Members Fields (Hidden by default, shown when members checkbox is checked) -->
                <div id="members-fields" class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3" style="display: <?php echo e(($filters['filter_members'] ?? false) ? 'grid' : 'none'); ?>;">
                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">الحد الأدنى لعدد الأفراد</label>
                        <input type="number" name="members_min" value="<?php echo e($filters['members_min'] ?? ''); ?>" min="1"
                            class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-700 mb-1">الحد الأقصى لعدد الأفراد</label>
                        <input type="number" name="members_max" value="<?php echo e($filters['members_max'] ?? ''); ?>" min="1"
                            class="w-full px-3 py-1.5 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-200">
                ...
                <?php if (\Illuminate\Support\Facades\Blade::check('hasPermission', 'beneficiaries.export_excel')): ?>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-900">قائمة المستفيدين (<?php echo e($beneficiaries->total()); ?>)</h2>
            <?php if($batch): ?>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="beneficiaries-table-body">
                    <?php echo $__env->make('beneficiaries.partials.rows', ['beneficiaries' => $beneficiaries, 'batch' => $batch ?? null], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                </tbody>
            </table>
        </div>
        ...
        <?php if($beneficiaries->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-200">
                <div class="text-sm text-gray-600 text-center mb-2">عرض <strong><?php echo e($beneficiaries->firstItem() ?? 0); ?></strong> إلى <strong><?php echo e($beneficiaries->lastItem() ?? 0); ?></strong> من <strong><?php echo e($beneficiaries->total()); ?></strong></div>
...
function exportExcel() {
    window.location.href = '<?php echo e(route('beneficiaries.export-excel')); ?>';
}
...
function clearFilters() {
    ...
    form.method = 'POST';
    form.action = '<?php echo e(route('beneficiaries.filter.reset')); ?>';
...
async function fetchAllFilteredIds() {
    try {
        ...
        const response = await fetch('<?php echo e(route("beneficiaries.get-filtered-ids")); ?>', {
            method: 'POST',
            headers: {
...
document.getElementById('add-to-batch-btn')?.addEventListener('click', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch('<?php echo e(route("batches.add-beneficiaries", $batch)); ?>', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '<?php echo e(route("batches.manage", $batch)); ?>';
                    }, 1000);
                } else {
...
uploadExcelBtn?.addEventListener('click', function() {
    ...
    fetch('<?php echo e(route("beneficiaries.import-preview")); ?>', {
        method: 'POST',
        body: formData
    })
    .then(r => r.json())
...
confirmExcelBtn?.addEventListener('click', function() {
    ...
    fetch('<?php echo e(route("beneficiaries.import")); ?>', {
        method: 'POST',
        body: formData
    })
    .then(r => r.json())
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\goodaid\resources\views/beneficiaries/index.blade.php ENDPATH**/ ?>