...
@section('title', 'توزيع الطرد: ' . $batch->name)
...
<div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h1 class="text-4xl font-bold text-gray-900">توزيع الطرد: {{ $batch->name }}</h1>
            <p class="text-gray-600 mt-2 text-lg">{{ $batch->type_name }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="text-lg font-semibold text-gray-900">{{ $batch->batch_date->format('Y-m-d') }}</p>
            </div>
        @endif
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2 total-count">{{ $totalBeneficiaries }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-green-600 mt-2 received-count">{{ $receivedCount }}</p>
                    <p class="text-sm text-gray-500 mt-1 received-percentage">{{ $receivedPercentage }}%</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-red-600 mt-2 not-received-count">{{ $notReceivedCount }}</p>
                    <p class="text-sm text-gray-500 mt-1 not-received-percentage">{{ $notReceivedPercentage }}%</p>
                </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="beneficiaries-table-body">
                    @forelse($beneficiaries as $index => $beneficiary)
                        ...
                        <tr class="beneficiary-row" 
                            ...
                            data-received="{{ $received ? 'received' : 'not_received' }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center">{{ $beneficiaries->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-center">{{ $beneficiary->name }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{{ $beneficiary->family_members_count + 1 }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center approved-by-cell">
                                @if($approvedBy)
                                    {{ \App\Models\User::find($approvedBy)->name ?? '-' }}
                                @else
                                    -
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center">
                                @if(!$received)
                                    <button type="button" 
                                        onclick="toggleReceived({{ $beneficiary->id }}, true)"
                                ...
                                @else
                                    <button type="button" 
                                        onclick="toggleReceived({{ $beneficiary->id }}, false)"
        ...
        @if($beneficiaries->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 mt-4">
                <div class="text-sm text-gray-600 text-center mb-2">
                    عرض <strong>{{ $beneficiaries->firstItem() ?? 0 }}</strong> إلى <strong>{{ $beneficiaries->lastItem() ?? 0 }}</strong> من <strong>{{ $beneficiaries->total() }}</strong>
                </div>
...
function toggleReceived(beneficiaryId, received) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`/batches/{{ $batch->id }}/recipients/${beneficiaryId}/toggle-received`, {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (received) {
                        ...
                        // تحديث اسم المعتمد
                        approvedByCell.textContent = '{{ auth()->user()->name }}';
                    } else {
...
function exportExcel() {
    ...
    let exportUrl = '{{ route("batches.distribution.export-excel", $batch) }}';
