    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="flex items-center">
                <a href="{{ route('beneficiary.dashboard') }}" class="flex items-center space-x-2 space-x-reverse">
            ...
            <div class="hidden md:flex items-center gap-6">
                @if($beneficiary)
                    <span class="text-gray-600 text-sm font-medium">{{ $beneficiary->name }}</span>
                @endif
                <form action="{{ route('beneficiary.logout') }}" method="POST" class="inline">
                    @csrf
    ...
    <div id="mobile-menu-beneficiary" class="hidden md:hidden fixed right-0 top-0 bottom-0 w-1/2 bg-white z-50 transform transition-transform duration-300 ease-out overflow-y-auto shadow-2xl">
        <div class="flex flex-col min-h-full">
            ...
            <div class="px-4 py-4 border-b border-gray-200 bg-white">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-gray-600">@if($beneficiary){{ $beneficiary->name }}@endif</div>
                    </div>
                </div>
            </div>
            ...
            <div class="px-4 py-3 border-t border-gray-200 bg-gray-50">
                <form action="{{ route('beneficiary.logout') }}" method="POST">
                    @csrf
